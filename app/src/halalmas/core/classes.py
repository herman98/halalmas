...
class ChoiceConstantBase:
    ...
    @classmethod
    def get_choices(cls, isUpperCase=None):
        choices = ()
        for attr_name in cls.ordering(dir(cls)):
            attr = getattr(cls, attr_name)
            if not callable(attr) and not attr_name.startswith('_'):
                if isUpperCase:
                    choices += ((attr, cls.coerce(attr_name.replace('___',
                                                                    ')').replace('__', '(').replace('_', ' ')).upper()),)
                else:
                    choices += ((attr, cls.coerce(attr_name.replace('___',
    ...
    @classmethod
    def get_keys(cls):
        return tuple(attr_name for attr_name in dir(cls)
