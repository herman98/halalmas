...
from halalmas.server.members.profiles.models import CustomerUserProfile
...
class ApiV1Backend(object):
    ...
    def authenticate(self, **request_attrs):
        ...
        #     username = '_'.join(
        #         [request_attrs['facebook_id'], str(int(time.time()))])
        # else:
    ...
    #     logger.info(
    #         "Token for facebook_id=%s verified" % request_attrs['facebook_id'])
    ...
    #     facebook_data_fields = [
    #         'facebook_id', 'fullname', 'email', 'dob', 'gender'
    #     ]
    #     attrs = {}
    ...
    #     user.clean_fields()
    #     logger.debug(
    #         "Validated inputs to register user username=%s" % username)
    ...
    #     for key in attrs:
    #         logger.debug(
    #             "Associating facebook data=%s with username=%s" % (
    #                 key, username))
